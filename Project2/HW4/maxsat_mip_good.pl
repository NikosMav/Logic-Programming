...
evaluate_literals([Packed_literal|Rest_packed_disjunction], Eval) :-
  Packed_literal = pack(Sign, CSP_var),
  Sign is 1,
  !,
  evaluate_literals(Rest_packed_disjunction, Rest_eval),
...
evaluate_disjunctions([Packed_disjunction|Rest_packed_formula], [Eval|Rest_evals]) :-
  evaluate_literals(Packed_disjunction, Literals_eval),
  Eval #:: [0, 1],
  Eval + Literals_eval #>= 1, /*if at least one is true, the disjunction is true.*/
  evaluate_disjunctions(Rest_packed_formula, Rest_evals).


/*the cost is the number of false disjunctions.*/
cost(Packed_formula, NC, Cost, Evals) :-
...
maxsat(NV, NC, D, F, S, M) :-
  ...
  writeln(Cost),
  !,
  bb_min(search([Evals|S], 0, input_order, indomain_min, complete, []), Cost, bb_options{strategy:dichotomic}),
  M is NC - Cost.
