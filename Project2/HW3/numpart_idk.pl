...
cannot possibly stay below it, so we would backtrack. Tested same logic for
sums of squares, overhead is not cost effective (600+ seconds for
 findall(32) on I5-7200U laptop, while it is 300ish without it). If I had to
 guess, I'd say doing the pruning with squares doesn't help much because
 [Lower_lim, Upper_lim] is too wide and we don't prune as much, plus the
 calculations are much more costly.*/


%this only really starts the process, does not add anything to SL1
make_and_constrain_partial_sums([X|L1], SL1, N, I, S1Goal) :-
  New_I is I + 1,
  make_and_constrain_partial_sums(L1, SL1, X, N, New_I, S1Goal).

%we leave last one out. We only want STRICTLY partial sums
make_and_constrain_partial_sums([X], [], _, _, _, _).

%makes a domain variable from the previous one plus the current problem variable
%the partial sum must be bounded in [Lower_lim, Upper_lim] as is explained above
make_and_constrain_partial_sums([X|L1], [Z|SL1], Y, N, I, S1Goal) :-
  Z #= X + Y,
  K is N // 2 - I, %K is equal to number of variables NOT in partial sum
  max_sum(K, N, Max_S),
  min_sum(K, Min_S),
